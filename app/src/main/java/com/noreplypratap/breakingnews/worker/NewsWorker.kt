...
import com.noreplypratap.breakingnews.notification.buildNotification
import com.noreplypratap.breakingnews.repository.Repository
import com.noreplypratap.breakingnews.ui.views.NewsActivity
...
class NewsWorker @AssistedInject constructor(
    ...
    override fun doWork(): Result {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            if (context.isOnline()) {
                val news = repository.loadArticles(codeIndia)
                if (news.isSuccessful) {
                    news.body()?.articles?.let { articles ->
    ...
    private fun Context.buildAndScheduleNotification(article: Article, intent: Intent): Notification {
    ...
    private fun updateNotification(id: Int, article: Article) {
