...
fun Context.showBottomSheetDialog(article: Article, btnStatus: Boolean = false, deleteArticle : () -> Unit = {},saveArticle : () -> Unit = {}) {
    ...
    if (checkForNull(article.source?.name)){
        view.findViewById<TextView>(R.id.articleSourceTextView).apply {
            visibility = View.VISIBLE
            text = "Source: ${article.source?.name.toString()}"
        }
    }
